...
#define stepsPerInchX 400
#define stepsPerInchY 400
#define stepsPerInchZ 400
...
#if stepperX
AccelStepper stprX(1, 2, 5);	//type, stepPin, dirPin
#endif
#if stepperY
AccelStepper stprY(1, 3, 6);
#endif
#if stepperZ
AccelStepper stprZ(1, 4, 7);
#endif
#if stepperA
...
unsigned long getDelta(int axis, long pos)
{
    ...
    long delta;
    if (pos < deltaPosOld[axis]) {
	delta = deltaPosOld[axis] - pos;
    } else {
	delta = pos - deltaPosOld[axis];
    }
    if (delta < 0) {
	delta *= -1;
    }
