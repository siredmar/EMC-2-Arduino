...
PID xPID(&Input[0], &Output[0], &Setpoint[0], Xproportional, Xproportional,
	 Xderivative, REVERSE);
#endif

#if YdirPin > 0 && YpwmPin > 0
PID yPID(&Input[1], &Output[1], &Setpoint[1], Yproportional, Yproportional,
	 Yderivative, REVERSE);
#endif

#if ZdirPin > 0 && ZpwmPin > 0
PID zPID(&Input[2], &Output[2], &Setpoint[2], Zproportional, Zproportional,
	 Zderivative, REVERSE);
#endif

#if AdirPin > 0 && ApwmPin > 0
PID aPID(&Input[3], &Output[3], &Setpoint[3], Aproportional, Aproportional,
	 Aderivative, REVERSE);
#endif

#if BdirPin > 0 && BpwmPin > 0
PID bPID(&Input[4], &Output[4], &Setpoint[4], Bproportional, Bproportional,
	 Bderivative, REVERSE);
#endif

#if CdirPin > 0 && CpwmPin > 0
PID cPID(&Input[5], &Output[5], &Setpoint[5], Cproportional, Cproportional,
	 Cderivative, REVERSE);
#endif

#if UdirPin > 0 && UpwmPin > 0
PID uPID(&Input[6], &Output[6], &Setpoint[6], Uproportional, Uproportional,
	 Uderivative, REVERSE);
#endif

#if VdirPin > 0 && VpwmPin > 0
PID vPID(&Input[7], &Output[7], &Setpoint[7], Vproportional, Vproportional,
	 Vderivative, REVERSE);
#endif

#if WdirPin > 0 && WpwmPin > 0
PID wPID(&Input[8], &Output[8], &Setpoint[8], Wproportional, Wproportional,
	 Wderivative, REVERSE);
#endif
...
void encoderServoSetup()
{
	...
	if (encServoDirPin[i] != 0) {
	    pinMode(encServoDirPin[i], OUTPUT);
	}
	if (encServoEnablePin[i] != 0) {
	    pinMode(encServoEnablePin[i], OUTPUT);
	}
...
void moveEncoderServosOld()
{
	...
	for (int i = 0; i < 2; i++) {
	    if (encServoDirPin[i] > 0) {
		long dist =
		    getDist((targetPos[i] / servoScale[i]),
			    encoderPosition[i]);
		if (dist > encServoDeadband[i]) {
		    if (encoderPosition[i] <
			(targetPos[i] / servoScale[i])) {
			digitalWrite(encServoDirPin[i],
				     !encServoInversion[i]);
		    } else {
			digitalWrite(encServoDirPin[i],
				     encServoInversion[i]);
		    }
		    digitalWrite(encServoEnablePin[i],
				 encServoEnversion[i]);
		    servoError[i]++;
		    analogWrite(encServoPwmPin[i],
				constrain(dist + servoError[i] * 2,
					  encServoMinPwm[i],
					  encServoMaxPwm[i]));
		} else {
